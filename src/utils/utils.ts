...
export function log_info() {
  ...
  if (Game.time % 100 === 0) {
    ...
    console.log(
      `[${Inscribe.color(Emoji.recycling + " | " + Config.FIRST_ROOM_NAME, "skyblue")}]| Game-Tick: ${Game.time}` +
        "| CPU Used: " +
        Game.cpu.getUsed() +
        "| Buck: " +
        Game.cpu.bucket
    );
    //log.info(Inscribe.color(Emoji.tick + " " + Config.FIRST_ROOM_NAME, "skyblue") + " | Game-Tick: " + Game.time + " | CPU Used: " + Game.cpu.getUsed() + " | Buck: "+ Game.cpu.bucket);
  }
}
